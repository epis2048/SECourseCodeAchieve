...
public class MainFrame extends JFrame {
	...
	private JMenuItem miGather, miSave, miSend,
			miShow, miExit, miCheck, miAbout, menuMatchLogRec, menuMatchTrans;
	...
	private void initMenu() {
		...
		menuMatchLogRec = new JMenuItem("匹配日志");
		menuMatchLogRec.addActionListener(new MatchLogListener());
		menuMatchTrans = new JMenuItem("匹配物流");
		menuMatchTrans.addActionListener(new MatchTransListener());
		menuMatch.add(menuMatchLogRec);
		menuMatch.add(menuMatchTrans);		
	...
	private void initToolBar() {
		...
		btnMatchLog = new JButton("匹配数据", matchIcon);
		// 注册监听
		btnMatchLog.addActionListener(new MatchLogListener());
		btnMatchLog.addActionListener(new MatchTransListener());
	...
	private void initLogGatherGUI() {
			...
			//添加初始ID
			txtLogId.setText(Integer.toString(logRecService.getLogMaxID()+1));
		}
	...
	private void initTransGatherGUI() {
		...
		//添加初始ID
		txtTransId.setText(Integer.toString(transportService.getLogMaxID()+1));
	}
		...
		public void actionPerformed(ActionEvent e) {
			if(txtLogId.getText().equals("")) {
				...
				if(txtLogId.getText().matches("\\d+")) {
					...
					txtLogId.setText(Integer.toString(id+1));
					txtLocation.setText("");
					txtName.setText("");
					txtIP.setText("");
					
				} else {
					JOptionPane.showMessageDialog(null,"ID必须是数字！","错误提示",JOptionPane.ERROR_MESSAGE);
				}
			}
		}
	}
	...
	private class GatherTransListener implements ActionListener {
		// 数据采集的事件处理方法
		public void actionPerformed(ActionEvent e) {
			if(txtTransId.getText().equals("")) {
				JOptionPane.showMessageDialog(null,"ID不能为空！","错误提示",JOptionPane.ERROR_MESSAGE);
			} else {
				if(txtTransId.getText().matches("\\d+")) {
					// 获取物流ID
					...
					JOptionPane.showMessageDialog(null, "物流采集成功！", "提示",
							JOptionPane.INFORMATION_MESSAGE);
					txtTransId.setText(Integer.toString(id+1));
					txtAdress.setText("");
					txtHandler.setText("");
					txtReceiver.setText("");
				} else {
	...
	private class SaveDataListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "日志数据保存成功！", "提示",
						JOptionPane.INFORMATION_MESSAGE);
							
				// 保存匹配的物流信息
				//若保存成功，弹出提示框：匹配的物流数据以保存到文件和数据库中！",
				//若没有保存成功，则弹出相应的告警提示框
				transportService.saveAndAppendMatchTransport(matchedTrans);
				//transportService.saveMatchTransportToDB(matchedTrans);
				JOptionPane.showMessageDialog(null, "物流数据保存成功！", "提示",
						JOptionPane.INFORMATION_MESSAGE);
			} catch(Exception ex){
				JOptionPane.showMessageDialog(null, "保存过程出错！", "提示",
	...
	private class SendDataListener implements ActionListener {
		// 数据发送的事件处理方法
		public void actionPerformed(ActionEvent e) {
			//JOptionPane.showMessageDialog(null, "1111！", "提示", JOptionPane.INFORMATION_MESSAGE);
			try {
				//日志
				if(matchedLogs != null && matchedLogs.size() > 0) {
					Socket logSocket = new Socket(serverIP, 12347);
					...
					matchedLogs.clear();
					logList.clear();
					JOptionPane.showMessageDialog(null, "匹配的日志已经发送到服务器！", "提示", JOptionPane.INFORMATION_MESSAGE);
				}
				else {
				...
				if(matchedTrans != null && matchedTrans.size() > 0) {
					Socket transSocket = new Socket(serverIP, 12344);
					...
					matchedTrans.clear();
					transList.clear();
					JOptionPane.showMessageDialog(null, "匹配的物流信息已经发送到服务器！", "提示", JOptionPane.INFORMATION_MESSAGE);
				}
				else {
					JOptionPane.showMessageDialog(null, "没有匹配的物流信息！", "提示", JOptionPane.WARNING_MESSAGE);
				}
			} catch(Exception e1) {
				e1.printStackTrace();
			}
	...
	private void flushMatchedTransTable() {
		...
		MatchedTableModel tranModel = new MatchedTableModel(
				transportService.readTranResult(), 0);
		// 使用tableModel创建JTable
